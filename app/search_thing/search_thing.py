...
def up_picture_url_1():
    if request.method == 'POST':
        ...
        if allowed_file(picture.filename):
            ...
            picture.save(os.path.join('''./app/static/search_pictures''', p_name))
        else:
            error = 'Type error'
            return jsonify({'status': 'error', 'error': error})
...
def insert():
    if request.method == 'POST':
        ...
        if picture_num >= 1 and allowed_file(picture_1.filename):
            ...
            picture_1.save(os.path.join('''./app/static/search_pictures''', p1_name))
        ...
        if picture_num >= 2 and allowed_file(picture_2.filename):
            ...
            picture_2.save(os.path.join('''./app/static/search_pictures''', p2_name))
        ...
        if picture_num >= 3 and allowed_file(picture_3.filename):
            ...
            picture_3.save(os.path.join('''./app/static/search_pictures''', p3_name))
...
def refresh_newest():
    if request.method == 'POST':
        ...
        json_data = list_2_json(
            SearchThing.query.filter_by(is_deleted=0).order_by(SearchThing.id.desc()).limit(10).all())
...
def get_user_posts():
    if request.method == 'POST':
        ...
        if error is None:
            json_data = list_2_json(
                SearchThing.query.filter_by(user_id=user_id).order_by(SearchThing.id.desc()).all())
